...
railsNetwork n seed = railsNetworkH n (getRandomNumbers (n*n) seed) 0

railsNetworkH :: Int -> [Int] -> Int -> [[Int]]
railsNetworkH n list x = 
    if (x < n) 
    then setDiagonalZero x (take n list) : railsNetworkH n (drop n list) (x + 1)
    else []

pullLever :: Int -> Int -> Int -> [[Int]] -> [[Int]]

pullLever seed intialStation nextStation network = take (intialStation-1) network  ++ [(leverNodeH seed 0 (nextStation-1) (network!!(intialStation-1)))] ++ drop (intialStation) network

leverNodeH :: Int -> Int -> Int -> [Int] -> [Int]
leverNodeH seed x nextStation (h:t)
    | x < nextStation = h : leverNodeH seed (x + 1) nextStation t
    | x == nextStation = (getRandomNumbers 1 seed) !! 0 : t
    | otherwise = []

setDiagonalZero :: Int -> [Int] -> [Int]
setDiagonalZero i row = take i row ++ [0] ++ drop (i + 1) row

initializeSource :: Int -> [[Int]] -> PriorityQueue
initializeSource n list = initializeSourceH n m 0 where m = length list

initializeSourceH :: Int -> Int -> Int -> PriorityQueue
initializeSourceH n m current 
    | current == 0 = PQ (Node n 0) (initializeSourceH n m (current + 1))
    | current < m && current /= n = PQ (Node current 9999) (initializeSourceH n m (current + 1))
